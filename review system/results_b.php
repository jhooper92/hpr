...
if (!$conn) {
	...
	if (isset($_POST['filterBy']) && !empty($_POST['filterBy']) && isset($_POST['searchFor']) && !empty($_POST['searchFor'])) {
		...
		$countsql = "SELECT COUNT(unique_id) FROM review_hold WHERE modStatus= '$filterBy' AND CCR_id= '$searchFor'";  

		$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE modStatus= '$filterBy' AND CCR_id= '$searchFor' ORDER BY reg_date DESC LIMIT $start_from, $limit");
	...
	elseif (isset($_POST['searchFor']) && !empty($_POST['searchFor'])) {
		...
		$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE CCR_id= '$searchFor' ORDER BY reg_date DESC LIMIT $start_from, $limit");
		$countsql = "SELECT COUNT(unique_id) FROM review_hold WHERE CCR_id= '$searchFor'"; 
	...
	elseif (isset($_POST['filterBy']) && !empty($_POST['filterBy']) ) {
		...
		$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE modStatus= '$filterBy' ORDER BY reg_date DESC LIMIT $start_from, $limit");
		$countsql = "SELECT COUNT(unique_id) FROM review_hold WHERE modStatus= '$filterBy'"; 
		...
		$result = mysqli_query($conn,"SELECT * FROM review_hold ORDER BY reg_date DESC LIMIT $start_from, $limit");
		$filteredComment = " ";

		if(isset($_COOKIE['search']) && (isset($_COOKIE['filter'])) ) {
			$cookieSearch = $_COOKIE['search'];
			$cookieFilter = $_COOKIE['filter'];
			$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE modStatus= '$cookieSearch' AND CCR_id= '$cookieFilter' ORDER BY reg_date DESC LIMIT $start_from, $limit");
			$filteredComment = "<p class='currentfilter'> Filtered By: " . $cookieSearch . "</p>";
		}

		if(isset($_COOKIE['search']) ) {
			$cookieSearch = $_COOKIE['search'];
			$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE modStatus= '$cookieSearch' ORDER BY reg_date DESC LIMIT $start_from, $limit");
			$filteredComment = "<p class='currentfilter'> Filtered By: " . $cookieSearch . "</p>";
		}

		if(isset($_COOKIE['filter']) ) {
			$cookieFilter = $_COOKIE['filter'];
			$result = mysqli_query($conn,"SELECT * FROM review_hold WHERE modStatus= '$cookieFilter' ORDER BY reg_date DESC LIMIT $start_from, $limit");
			$filteredComment = "<p class='currentfilter'> Filtered By: " . $cookieFilter . "</p>";
		}
		
		$countsql = "SELECT COUNT(unique_id) FROM review_hold"; 
	}
...
<div class="row">
	...
	<?php
		if (isset($_POST['filterBy']) && !empty($_POST['filterBy']) || isset($_POST['searchFor']) && !empty($_POST['searchFor']) || isset($_COOKIE['filter'])) {
...
<?php
    ...
    while($row = mysqli_fetch_array($result)) {
    	...
    	echo "<div class='row'><p>Review ID:<span> " . $row['unique_id'] . "</span> | Product Code:<span> " . $row['CCR_id'] . "</span></p><p> Rating:<span> " . $row['rating'] . "</span></p><p>Nickname:<span> " . $row['nickname'] . "</span></p><p> Submission Date:<span>" . $row['reg_date'] . "</p><p> Current Status:<span> " . $modStatus . "</span></p>" ;
    			...
    			echo "<div class='description'> <p>" . $row['message'] . "</p></div>"; 
